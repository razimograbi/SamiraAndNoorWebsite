class SimplexGreedy {
    constructor() {
      ...
      this.B = Array.from({ length: this.N }, () => Array(this.N).fill(0));
      this.C = Array(this.N).fill(0);
      this.D = Array.from({ length: this.N }, () => Array(this.N).fill(0));
      this.c = Array(this.N).fill(0);
      this.b = Array(this.N).fill(0);
      this.cb = Array(this.N).fill(0);
      this.cbBI = Array(this.N).fill(0);
      this.cbBID = Array(this.M).fill(0);
      this.cd = Array(this.M).fill(0);
      this.rd = Array(this.M).fill(0);
      this.W = Array.from({ length: this.N }, () => Array(this.N).fill(0));
      this.BI = Array.from({ length: this.M }, () => Array(this.N).fill(0));
      this.BIA_aux = Array.from({ length: this.N }, () => Array(this.N).fill(0));
      this.A_aux = Array.from({ length: this.N }, () => Array(this.N).fill(0));
      this.BIb = Array(this.N).fill(0);
      this.d = Array(this.N).fill(0);
      this.basis = Array(this.N).fill(0);
      this.n = 0;
      this.m = 0;
      this.epsilon = 0;
      this.Initial_n = 0;
      this.Initial_cb = Array(this.N).fill(0);
      this.Initial_cd = Array(this.N).fill(0);
      this.Initial_A = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
      this.Initial_A_aux = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
      this.Initial_c = Array(this.N).fill(0);
      this.Initial_basis = Array(this.N).fill(0);
      this.Initial_D = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
      this.Initial_d = Array(this.N).fill(0);
      this.Initial_B = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
      this.Initial_BIb = Array(this.N).fill(0);
      this.Initial_b = Array(this.N).fill(0);
      this.Initial_b_aux = Array(this.N).fill(0);
      this.Initial_rd = Array(this.N).fill(0);
      this.Initial_BI = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
      this.Initial_cbBI = Array(this.N).fill(0);
      this.Initial_cbBID = Array(this.N).fill(0);
      this.Initial_BIA_aux = Array.from({ length: this.N }, () =>
        Array(this.N).fill(0)
      );
    }
      ...
      while (optimal_flag == 0) {
        ...
        */
        ...
        if (min_value >= 0.0) {
          ...
          let enter_id = this.findMostNegative();
          ...
          if(exiting_id instanceof Object && exiting_id.hasOwnProperty('msg')){
            return "Unbounded linear program!";
          }
    ...
    findMostNegative() {
      let most_index = this.d[this.m];
      let temp_value = this.rd[0];
  
      for (let i = 0; i < this.n - this.m; ++i) {
        if (this.rd[i] < temp_value) {
          most_index = this.d[this.m + i];
          temp_value = this.rd[i];
        }
      }
